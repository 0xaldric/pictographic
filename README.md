...
- Constraint: **no third-party libraries** — everything from PNG decoding to the skeleton is hand-written.
...
├── README.md
├── centerline.py        # everything, stdlib only
│   ├── read_png(path)                 # Step 1  (chunk parse + zlib + de-filter)
│   ├── to_mask(...)                   # Step 2
│   ├── zhang_suen(W, grid)            # Step 3
│   ├── cleanup_skeleton(W, skel)      # Step 3b (remove staircase pixels)
│   ├── distance_transform(...)        # aux (Step 6)
│   ├── trace_branches(W, skel)        # Steps 4+5 (graph + trace, uses crossing number)
│   ├── prune(branches, deg, ...)      # Step 6
│   └── to_svg(branches, ...)          # Step 7
└── out/                 # result SVGs
```
Run: `python3 centerline.py challenge_sample/letter_H.png out/letter_H.svg`
(or pass a folder for both arguments).
...
- **Arrows (arrow-pointer):** the medial axis of a **solid triangle** is a **Y** (three spokes to
  ...
  leave a single axis.
- **Diagonal strokes (K, &):** Zhang–Suen leaves staircasing → the skeleton is slightly bumpy. The
  reference does **not** simplify, so we don't have to either; for smoother output one could add RDP
  (trade-off: diverges from the reference).
- **Junction position:** can shift by a few px depending on the thinning algorithm — acceptable.
...
- Replace Zhang–Suen with **Guo–Hall** or a **distance-transform + ridge** method for a smoother,
  spur-free skeleton.
- **Smooth** the polylines (RDP + Chaikin) and fit circular arcs on curved strokes.
- Smarter spur pruning using the **DT** (real spur vs real short stroke) instead of length alone.
- Reconstruct junctions geometrically (intersection of stroke axes) instead of the pixel-cluster
  centroid.
...
**Implemented and working** in `centerline.py` — all 7 sample shapes produce correct centerlines
(see Results). The code follows the steps in section 2 one function per step, so each part is
readable and independently verifiable.

Regenerate everything:
```
python3 centerline.py challenge_sample/ out/
```
